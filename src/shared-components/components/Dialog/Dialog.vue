<template>
  <v-dialog
    :value="value"
    content-class="pri-dialog-parent"
    max-width="576px"
    persistent
    width="90%"
  >
    <div :class="co_dialogColorClass" class="pri-dialog">
      <v-card tile>
        <v-card-title class="headline" v-if="content.title">
          <v-icon :color="co_dialogColor" :size="22" left>
            {{ co_dialogIcon }}
          </v-icon>
...
<script>
//TODO: Can be refactored like Message.vue to use v-alert
export default {
  name: "pri-dialog",
  props: {
    value: Boolean,
...
<style lang="scss">
.left-drawer-opened .pri-dialog-parent {
  margin-left: 255px;
}
